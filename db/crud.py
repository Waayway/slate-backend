...
def get_user_by_name(db: Session, name: str):
    return schemas.User.from_orm(db.query(models.User).filter(models.User.username == name).first())
...
def get_user_by_token(db: Session, token: str):
    return schemas.User.from_orm(db.query(models.User).filter(models.User.token == token).first())
...
def create_user(db: Session, user: schemas.UserCreate):
    ...
    db_user = models.User(username=user.username, email=user.email,
                          permissions=user.permissions, password=password)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    print(db_user.id)
...
def get_parent(db: Session, parent_id: int):
    return schemas.Parent.from_orm(db.query(models.ParentNote).filter(models.ParentNote.id == parent_id).first())


def get_parent_by_name(db: Session, name: str):
    return schemas.Parent.from_orm(db.query(models.ParentNote).filter(models.ParentNote.name == name).first())
...
def create_parent(db: Session, parent: schemas.ParentCreate):
    db_parent = models.ParentNote(name=parent.name, content=parent.content)
    db.add(db_parent)
    db.commit()
...
def delete_parent(db: Session, parent_id: int):
    db_parent = db.query(models.ParentNote).filter(
...
def get_note(db: Session, note_id: int):
    try:
        return schemas.Note.from_orm(db.query(models.Note).filter(models.Note.id == note_id).first())
    except ValidationError:
        return None


def get_note_by_name(db: Session, name: str):
    return schemas.Note.from_orm(db.query(models.Note).filter(models.Note.name == name).first())
...
def create_note(db: Session, note: schemas.NoteCreate):
    db_note = models.Note(name=note.name, type=note.type, content=note.content,
                          permission=note.permission, createdate=note.createdate, updatedate=note.createdate, owner_id=note.owner_id)
    db.add(db_note)
    db.commit()
    db.refresh(db_note)
