...
def addGetRequests(app: FastAPI):
    ...
    def get_parent_by_id(id: int):
        ...
        if parent:
            return JSONResponse({"data": parent, "message": "OK"}, status_code=status.HTTP_302_FOUND)
        else:
            return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content="None")
    ...
    def get_note_by_id(id: int):
        ...
        if note:
            return JSONResponse({"data": json.loads(note.json()), "message": "OK"}, status_code=status.HTTP_302_FOUND)
        else:
            return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content="None")
    ...
    def get_parent_from_note(id: int):
        ...
        if not note:
            ...
            if note.parent_id:
                ...
                if parent:
                    return JSONResponse({"data": json.loads(parent.json()), "message": "OK"}, status_code=status.HTTP_302_FOUND)
            return JSONResponse(status_code=status.HTTP_404_NOT_FOUND, content="None")
